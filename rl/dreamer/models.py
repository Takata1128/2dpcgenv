...
class TransitionModel(nn.Module):
    ...
    def prior(self, rnn_hidden):
        ...
        stddev = F.softplus(self.fc_state_stddev_prior(
...
class ObservationModel(nn.Module):
    """
    p(o_t | s_t, h_t)
    低次元の状態表現から画像を再構成するデコーダ (3, 64, 128)
    """
    ...
    def __init__(self, state_dim, rnn_hidden_dim):
        ...
        self.dc1 = nn.ConvTranspose2d(
            1024, 128, kernel_size=(4, 8), stride=1, padding=0)
        ...
        self.dc4 = nn.ConvTranspose2d(
            32, 16, kernel_size=(4, 4), stride=2, padding=1)
        self.dc5 = nn.ConvTranspose2d(
            16, 3, kernel_size=(4, 4), stride=2, padding=1)
...
class Encoder(nn.Module):
    """
    (3, 64, 128)の画像を(1024,)のベクトルに変換するエンコーダ
    """

    def __init__(self):
        super(Encoder, self).__init__()
        self.cv1 = nn.Conv2d(3, 16, kernel_size=4, stride=2,
                             padding=1)  # (64,128) -> (32,64)
        self.cv2 = nn.Conv2d(16, 32, kernel_size=4, stride=2,
                             padding=1)  # (32,64) -> (16,32)
        self.cv3 = nn.Conv2d(32, 64, kernel_size=4, stride=2,
                             padding=1)  # (16,32) -> (8,16)
        self.cv4 = nn.Conv2d(64, 128, kernel_size=4, stride=2,
                             padding=1)  # (8,16) -> (4,8)
        self.linear = nn.Linear(128 * 4 * 8, 1024)

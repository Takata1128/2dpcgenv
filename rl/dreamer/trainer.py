...
class Trainer:
    def __init__(self, env, config: DreamerConfig, train=True):
        # リプレイバッファの宣言
        ...
        self.rssm = RSSM(self.state_dim, self.action_dim,
                         self.rnn_hidden_dim, self.device)
        self.value_model = ValueModel(
            self.state_dim, self.rnn_hidden_dim).to(self.device)
        ...
        self.model_optimizer = torch.optim.Adam(
            self.model_params, lr=model_lr, eps=eps)
    ...
    def train(self):
        with wandb.init(project=self.config.project, config=self.config.__dict__):
            ...
            for episode in range(self.seed_episodes, self.all_episodes):
                ...
                start = time.time()
                # 行動を決定するためのエージェントを宣言
                policy = Agent(self.encoder, self.rssm.transition,
                               self.action_model)
                ...
                while not done:
                    ...
                    action += np.random.normal(
                        0, np.sqrt(
                            self.action_noise_var), self.env.action_space.shape[0]
                    )
                    next_obs, reward, done, _ = self.env.step(action)
                    self.replay_buffer.push(obs, action, reward, done)
                ...
                for update_step in range(self.collect_interval):
                    ...
                    actions = torch.as_tensor(
                        actions, device=self.device).transpose(0, 1)
                    rewards = torch.as_tensor(
                        rewards, device=self.device).transpose(0, 1)
                    ...
                    flatten_rnn_hiddens = rnn_hiddens.view(
                    ...
                    reward_loss = 0.5 * \
                        F.mse_loss(predicted_rewards, rewards[:-1])
                    ...
                    flatten_imaginated_states = imaginated_states.view(
                        -1, self.state_dim)
                    ...
                    clip_grad_norm_(self.action_model.parameters(),
                                    self.clip_grad_norm)
                    ...
                    value_loss.backward()
                    clip_grad_norm_(self.value_model.parameters(),
                                    self.clip_grad_norm)
                    ...
                    metrics = {}
                    metrics['model loss'] = model_loss.item()
                    metrics['kl loss'] = kl_loss.item()
                    metrics['obs loss'] = obs_loss.item()
                    metrics['reward loss'] = reward_loss.item()
                    metrics['value loss'] = value_loss.item()
                    metrics['action loss'] = action_loss.item()
                    wandb.log(metrics, step=total_update_step)
                ...
                if (episode + 1) % self.test_interval == 0:
                    policy = Agent(
                        self.encoder, self.rssm.transition, self.action_model)
                    start = time.time()
                    obs = self.env.reset()
                    done = False
                    total_reward = 0
                    while not done:
                        ...
                        total_reward += reward
                    wandb.log(
                        {"Episode Reward (Test)": total_reward})
                    print(
                        "Total test reward at episode [%4d/%4d] is %f"
                        % (episode + 1, self.all_episodes, total_reward)
                    )
                    print("elasped time for test: %.2fs" %
                          (time.time() - start))
    ...
    def calc_good_episodes(self, episode_count):
        ...
        while not done:
            action, pred_collision = policy.act_with_collision(
                obs, training=False)
    ...
    def compare_imagination(self, compare_count):
        ...
        for i in range(compare_count):
            ...
            state = self.rssm.transition.posterior(
                rnn_hidden, embedded_obs).sample()
            frame = np.zeros((64, 128 * 2, 3), dtype=np.uint8)
            frames = []
